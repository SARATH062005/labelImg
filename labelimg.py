...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, default_filename=None, default_prefdef_class_file=None, default_save_dir=None):
        ...
        self.dirty = False
        self.template_shape = None  # For storing the template bounding box
        ...
        # Template actions
        use_as_template = action('Use as Template for All', self.use_shape_as_template,
                                 'Ctrl+T', 'copy', 'Use selected bounding box as a template for all other images',
                                 enabled=False)
        clear_template = action('Clear Template', self.clear_template,
                                'Ctrl+Shift+T', 'close', 'Clear the currently stored template',
                                enabled=False)
        ...
        self.actions = Struct(save=save, save_format=save_format, saveAs=save_as, open=open, close=close, resetAll=reset_all, deleteImg=delete_image,
                              ...
                              useAsTemplate=use_as_template, clearTemplate=clear_template,
                              fileMenuActions=(
                                  open, open_dir, save, save_as, close, reset_all, quit),
                              beginner=(), advanced=(),
                              editMenu=(edit, copy, delete,
                                        None, color1, self.draw_squares_option,
                                        None, use_as_template, clear_template),
                              beginnerContext=(create, edit, copy, delete, use_as_template),
                              advancedContext=(create_mode, edit_mode, edit, copy,
                                               delete, shape_line_color, shape_fill_color, use_as_template),
                              onLoadActive=(
    ...
    def shape_selection_changed(self, selected=False):
        ...
        self.actions.useAsTemplate.setEnabled(selected)
    ...
    def load_file(self, file_path=None):
        ...
        if unicode_file_path and os.path.exists(unicode_file_path):
            ...
            # Add template shape if no shapes are loaded from file and a template is set
            if self.no_shapes() and self.template_shape:
                shape_data = (
                    self.template_shape['label'],
                    self.template_shape['points'],
                    self.template_shape['line_color'],
                    self.template_shape['fill_color'],
                    self.template_shape['difficult']
                )
                self.load_labels([shape_data])
                self.set_dirty()
    ...
    def show_bounding_box_from_annotation_file(self, file_path):
        if not file_path:
            print("No file path set. Please open an image first.")
            return
    ...
    def use_shape_as_template(self):
        if self.canvas.selected_shape:
            s = self.canvas.selected_shape
            # We need to store a copy of the points
            points_copy = [(p.x(), p.y()) for p in s.points]
            self.template_shape = dict(
                label=s.label,
                points=points_copy,
                line_color=s.line_color.getRgb(),
                fill_color=s.fill_color.getRgb(),
            ...
            self.actions.clearTemplate.setEnabled(True)
            self.statusBar().showMessage('Shape set as template for subsequent images.')
        else:
            self.statusBar().showMessage('No shape selected to use as a template.')

    def clear_template(self):
        self.template_shape = None
        self.actions.clearTemplate.setEnabled(False)
